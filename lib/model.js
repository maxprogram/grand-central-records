...
fn.toJSON = function() {
    ...
    var json = _(schema).reduce(function(json, val, key) {
        if (_(this).has(key) && _(this._original).has(key))
            json[key] = this[key];
...
fn.changed = function() {
    var now = this.toJSON();
    if (this[this._options._idField] == -1) return now;

    return _(this._original).reduce(function(changed, val, key) {
        if (val !== now[key]) changed[key] = now[key];
        return changed;
    }, {});
};
