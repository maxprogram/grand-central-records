...
function addChain(name, options) {
    fn[name] = function() {
        ...
        if (_.contains(options.notFor, this.adapter)) {
            if (callback) return this._query(callback);
            else return this;
        }
...
//==========================================================
// #find()
// USAGE: #find(id int || int[])

addChain('find', {
    use: 'where',
    constructor: function(id) {
        if (id === null || id === undefined) {
            log.error('#find() needs a valid ID');
            return ['###'];
        }

        if (Array.isArray(id)) id = id.join(",");
        else if (isNaN(parseFloat(id))) {
            log.error('#find() should be an array or number');
            return ['###'];
        }

        return [this.idField + ' IN ('+id+')'];
    }
});
...
addChain('remove', {
    ...
    constructor: function(id) {
        ...
        if (where) {
            ...
            log.error(".where() must be called before .remove()");
            return '###';
        }
    }
});
