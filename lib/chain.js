...
/** Utility to add integrated chain method to GCR
 * @param {string} name    Method name
 * @param {object} options
 *        {string}   .action|from|where|table|orderBy|limit|offset|returning
 *                                Defines the query attribute
 *        {function} .constructor Fn returning string to add to query attribute
 *        {string}   .use         Name of other chain method to use
 *        {[string]} .notFor      List of incompatible engines
 */
...
//==========================================================
// #select()
// USAGE: #select(string || string[] || {alias: column})
...
addChain('select', {
    ...
    constructor: function(columns) {
        ...
        // If object, attach aliases
        if (_.isPlainObject(columns)) {
            columns = _.map(columns, function(col, alias) {
                return col + ' AS ' + alias;
            });
        }

        // Join & add table name to columns
        columns = _.isString(columns) ? _.compact(columns.split(',')) : columns;
...
addChain('orderBy', {
    use: 'order',
    constructor: function(columns) { return columns; }
});
...
exports.findOne = function(id) {
    return this.find(id).then(function(res) { return res[0]; });
};

exports.findByIdAndUpdate = function(id, data) {
